...
const Indent = () => {
  ...
  const fetchData = async () => {
    ...
    const iRes = await fetch(`http://localhost:5000/indent?projectId=${activeProject.id}`).then(res => res.json());
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!activeProject) return;
    await fetch('http://localhost:5000/indent', {
      method: 'POST',
  ...
  const handleStatus = async (id, status) => {
    await fetch(`http://localhost:5000/indent/${id}/status`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
