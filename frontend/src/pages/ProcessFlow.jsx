  ...
  const buildGraph = useCallback(async () => {
    ...
    try {
      const [vRes, poRes, bRes] = await Promise.all([
        fetch('${import.meta.env.VITE_API_URL || "http://localhost:5000"}/vendors').then(r => r.json()),
        fetch('${import.meta.env.VITE_API_URL || "http://localhost:5000"}/po').then(r => r.json()),
        fetch('${import.meta.env.VITE_API_URL || "http://localhost:5000"}/bills').then(r => r.json()),
      ]);
